    ...
    public class ShoppingCartController : Controller
    {
        ...
        private Payment CreatePayment(APIContext apiContext, string redirectUrl)
        {
            ...
            foreach (var item in cart.Items)
            {
                ...
                // Chuyển đổi giá trị VND sang USD
                decimal usdPrice = item.Price / exchangeRate;

                // Làm tròn giá trị USD
                decimal roundedUsdPrice = Math.Round(usdPrice, 2);

                itemList.items.Add(new Item()
                {
                    name = item.ProductName,
                    currency = "USD",
                    price = roundedUsdPrice.ToString("F2"), // Định dạng giá trị tiền tệ với 2 chữ số thập phân
